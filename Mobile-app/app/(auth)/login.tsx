...
const Login = () => {
  ...
  const handleLoginP = async () => {
    ...
    try {
      setLoading(true);
      const response = await axios.post('http://192.168.8.153:5001/loginP', {
        emailAddress,
        password1
      });
      setLoading(false);
      ...
      const userDataResponseP = await axios.get('http://192.168.8.153:5001/get-user-dataP', {
        params: { emailAddress}
      });
...
const handleLoginT = async () => {
  ...
  try {
    ...
    const response = await axios.post('http://192.168.8.153:5001/loginT', {
